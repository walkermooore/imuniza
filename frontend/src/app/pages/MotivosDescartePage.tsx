...
import { Trash, Pencil, Plus, Trash2, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react'
import { useAuthStore } from '../../stores/auth'
import { apiClient } from '../../lib/api-client'
import type { BottleDiscardReason, PaginatedMeta, PaginatedResponse } from '../../types'
...
interface ApiBottleDiscardReason {
  id: string
  name: string
  is_default: boolean
  is_deleted: boolean
...
function mapApiReason(a: ApiBottleDiscardReason): BottleDiscardReason {
  return {
    id: a.id,
    name: a.name,
...
export default function MotivosDescartePage() {
  ...
  const [reasons, setReasons] = useState<BottleDiscardReason[]>([])
  ...
  const [editTarget, setEditTarget] = useState<BottleDiscardReason | null>(null)
  const [editName, setEditName] = useState('')
  const [editIsDefault, setEditIsDefault] = useState(false)
  const [editError, setEditError] = useState('')
  const [editSaving, setEditSaving] = useState(false)

  const [deleteTarget, setDeleteTarget] = useState<BottleDiscardReason | null>(null)
  ...
  const fetchReasons = useCallback(async (p: number) => {
    ...
    try {
      ...
      const res = await apiClient.get<PaginatedResponse<ApiBottleDiscardReason>>(
        `/bottle-discard-reasons?${params}`,
      )
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    try {
      await apiClient.post('/bottle-discard-reasons', {
        name: formName.trim(),
        is_default: formIsDefault,
      })
      toast({ title: 'Motivo de descarte criado com sucesso.' })
      setSheetOpen(false)
  ...
  function openEdit(r: BottleDiscardReason) {
    setEditTarget(r)
    setEditName(r.name)
  ...
  async function handleEdit() {
    ...
    setEditSaving(true)
    try {
      await apiClient.put(`/bottle-discard-reasons/${editTarget.id}`, {
        name: editName.trim(),
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await apiClient.delete(`/bottle-discard-reasons/${deleteTarget.id}`)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Motivos de Descarte</h1>
          <p className="text-sm text-muted-foreground">Gerencie os motivos disponíveis para descarte de frascos.</p>
        </div>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="sm:max-w-md">
          <SheetHeader>
            <SheetTitle>Novo motivo de descarte</SheetTitle>
      ...
      <Sheet open={!!editTarget} onOpenChange={(open) => { if (!open) setEditTarget(null) }}>
        <SheetContent className="sm:max-w-md">
          <SheetHeader>
            <SheetTitle>Editar motivo de descarte</SheetTitle>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null) }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Excluir motivo de descarte</DialogTitle>
            <DialogDescription>
